...
constructor(
  ...
  private suspend fun writeRawFace(
    ...
    return try {
      val ok: Boolean =
        FileOutputStream(tempDest).use writeTempRawFace@{ outputStream: FileOutputStream ->
          val wroteFace: Boolean = writeSmoothedRawFace(faceIndex, source, outputStream)
          if (wroteFace) {
            outputStream.fd.sync()
          }
          return@writeTempRawFace wroteFace
        }
      if (!ok) {
        tempDest.delete()
        return false
      ...
      if (!tempDest.renameTo(dest)) {
        Log.e(TAG, "Failed publishing $dest")
        tempDest.delete()
        return false
      }
      true
  ...
  companion object {
    ...
    private const val CLOUD_FEATHER_WEIGHT: Int =
      CLOUD_FEATHER_BLUR_WEIGHT + CLOUD_FEATHER_MAX_WEIGHT
