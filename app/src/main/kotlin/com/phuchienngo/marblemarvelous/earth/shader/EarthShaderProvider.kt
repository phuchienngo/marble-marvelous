...
class EarthShaderProvider : DefaultShaderProvider() {
  ...
  private class EarthShader(
    ...
    override fun render(renderable: Renderable) {
      val pointLights: PointLightsAttribute =
        renderable.environment.get(PointLightsAttribute.Type) as PointLightsAttribute
      val sunLight: PointLight = pointLights.lights.first()
      val day: EarthTextureAttribute =
        renderable.material.get(EarthTextureAttribute.DAY_DIFFUSE) as EarthTextureAttribute
      val night: EarthTextureAttribute =
        renderable.material.get(EarthTextureAttribute.NIGHT_DIFFUSE) as EarthTextureAttribute
      val cloudMask: CubemapAttribute =
        renderable.environment.get(CubemapAttribute.EnvironmentMap) as CubemapAttribute
      ...
      if (tmpMatrix.det() == 0.0f) {
        Console.warn(
          TAG,
          "Can't invert matrix because it does not have a determinant, skip this frame"
        )
        return
      }
  ...
  companion object {
    private const val TAG: String = "EarthShader"
  }
}
