...
import com.phuchienngo.marblemarvelous.celestialBodies.core.LightningStorms
import com.phuchienngo.marblemarvelous.celestialBodies.core.Stars
import com.phuchienngo.marblemarvelous.celestialBodies.shaders.EarthShaderProvider
...
import com.phuchienngo.marblemarvelous.weather.StormsProvider
...
class EarthEngine(
    ...
    private val stormsProviderFactory: StormsProvider.Factory,
    ...
    private var storms: LightningStorms? = null
    private var stormsProvider: StormsProvider? = null
    ...
    override fun initialize() {
        ...
        stormsProvider = stormsProviderFactory.create(this)
        stars = Stars(cam)
        storms = LightningStorms(stormsProvider!!.getLatestStorms())
    ...
    override fun resume() {
        ...
        if (isLoaded()) {
            ...
            if (!stormsProvider!!.isDataUpdated()) {
                stormsProvider!!.requestStormsUpdate()
            }
        }
    }
    ...
    override fun dispose() {
        ...
        val currentStormsProvider: StormsProvider? = stormsProvider
        if (currentStormsProvider != null) {
            currentStormsProvider.dispose()
            stormsProvider = null
        }
        ...
        val currentStorms: LightningStorms? = storms
        if (currentStorms != null) {
            currentStorms.dispose()
            storms = null
        }
        batch?.dispose()
        batch = null
    ...
    override fun onCloudsUpdated() {
        ...
        if (storms != null) {
            storms!!.setStormsLocations(null)
            stormsProvider!!.requestStormsUpdate()
        } else {
            stormsProvider!!.setNeedsUpdate()
        }
    }

    @Synchronized
    override fun onStormsUpdated(stormLocations: StormProtos.StormLocations?) {
        storms?.setStormsLocations(stormLocations)
    }
    ...
    private fun doneLoading() {
        ...
        mEarth!!.materials.first().set(PlanetTextureAttribute.createNight(nightDiffuse))
        mEarth!!.materials.first().set(PlanetTextureAttribute.createStorms(storms!!.getStormsTexture()))
        if (nextCloudMap == null) {
    ...
    protected fun setEarthAndCameraPosition(date: Date) {
        ...
        if (newSeason != season) {
            ...
            mEarth!!.materials.first().set(PlanetTextureAttribute.createDay(diffuse))
        }
    ...
    class Factory
        ...
        constructor(
            ...
            fun create(): EarthEngine =
                EarthEngine(context, cloudsProviderFactory, stormsProviderFactory, userLocation, inputMultiplexer, fpsThrottler)
        }
}
