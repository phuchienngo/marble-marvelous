package com.phuchienngo.marblemarvelous.controllers
...
class PageSwipeController {
    ...
    fun isScrollAnimating(threshold: Float): Boolean =
        abs(lastOffset - easedOffset) > threshold

    fun setPageSwipe(xOffset: Float, xOffsetStep: Float) {
    ...
    fun differenceBiggerThan(value: Float): Boolean =
        abs(lastOffset - easedOffset) > value
