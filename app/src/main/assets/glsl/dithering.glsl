...
uniform vec2 uResolution; // (1/width, 1/height), inverse of the surface's dimensions
...
vec4 Dither_InterleavedGradientNoise(vec4 rgba) {
    float noise = interleavedGradientNoise(gl_FragCoord.xy + uTime);
...
vec4 Dither_Vlachos(vec4 rgba) {
    highp vec3 noise = vec3(dot(vec2(171.0, 231.0), gl_FragCoord.xy + uTime));
...
vec4 Dither_TriangleNoise(vec4 rgba) {
    return rgba + triangleNoise(gl_FragCoord.xy * uResolution.xy) / 255.0;
}
...
vec4 dither(vec4 rgba) {
    // dithers the specified rgb color based on the current time and fragment coordinates
    // the input must be in the final color space (including OECF)
    // this dithering assumes we are dithering to an 8-bit target
